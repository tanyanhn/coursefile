...
\begin{frame}
  \begin{emt}[将总的问题切分为几部分]
    \begin{enumerate}
        ...
        \item 
        % 假设地震波在地层内部匀速直线传播,在穿过相邻地层之间的界面时发生折射.
...
\begin{frame}
    \begin{emt}[背景与意义\cite{2020Boolean}]
        \begin{itemize}
            ...
            在多相流领域中,一直在避免对流体建模.但是随着多相流研究的迅速发展，
...
\begin{frame}
    \begin{emt}[曲面片]
        ...
        \begin{itemize}
            ...
            \item 所以使用曲面片作为表示殷集边界的基本元, 曲面片内部点是邻域只包含一个圆盘
            的平凡情况,而边界是所有奇异点.
...
\begin{frame}
    \begin{emt}[黏合紧曲面]
        上述定理说明$\partial Y$可以通过黏合紧曲面的集合表示,
         又因为黏合紧曲面定义中不能是两个黏合紧曲面的粘合结果.
        黏合紧曲面表示$\partial Y$是唯一的.又因为殷集和殷集边界一一对应,
        所以黏合紧曲面的集合唯一表示殷集$Y$.
    \end{emt}

    \begin{emt}[从曲面片到黏合紧曲面]
        % 沿曲面片边界粘合得到黏合紧曲面的过程可以从黏合紧曲面定义中将一维子集删除后该黏合紧曲面
        % 仍然是连通的和不能剪开得到两个黏合紧曲面出发,
        从黏合紧曲面的定义出发,总结两条粘合准则

        \begin{itemize}
            \item 对每张曲面片的每条边界,若曲面片包含边界点的邻域内两个或更多广义扇形.选取是好配对的
            两个扇形粘合起来.
...
\begin{frame}
    \begin{emt}[意义\cite{wangzhan_1589}]
        \begin{itemize}
            ...
            \item 还能直接应用于内波对各种海洋结构物和部件(平台\caesura{}立管\caesura{}锚链等)的作用力计算,
            是进一步分析海洋结构物的响应\caesura{}强度和疲劳的前提和基础.
