...
!       send b to each other process
         do i = 1,bcols
         call MPI_BCAST(b(1,i), brows, MPI_DOUBLE_PRECISION, master,        MPI_COMM_WORLD, ierr)
         enddo
...
!       send a row of a to each other process; tag with row number
         do i = 1,numprocs-1
            do j = 1,acols
               buffer(j) = a(i,j)
            enddo
            call MPI_SEND(buffer, acols, MPI_DOUBLE_PRECISION, i,           i, MPI_COMM_WORLD, ierr)
            numsent = numsent+1
         enddo

         do i = 1,crows
            call MPI_RECV(ans, ccols, MPI_DOUBLE_PRECISION,           MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, status,ierr)
            ...
            if (numsent .lt. arows) then
               ...
               enddo
               call MPI_SEND(buffer, acols, MPI_DOUBLE_PRECISION,              sender, numsent+1, MPI_COMM_WORLD, ierr)
               numsent = numsent+1
            else
               call MPI_SEND(1.0, 1, MPI_DOUBLE_PRECISION, sender,              0, MPI_COMM_WORLD, ierr)
            endif
         enddo
...
!       slaves receive b, then compute rows of !until done message
         do i = 1,bcols
           call MPI_BCAST(b(1,i), brows, MPI_DOUBLE_PRECISION, master, MPI_COMM_WORLD, ierr)
         enddo
         do
            call MPI_RECV(buffer, acols, MPI_DOUBLE_PRECISION, master,   MPI_ANY_TAG, MPI_COMM_WORLD, status, ierr)
            ...
            do i = 1,bcols
               ...
               enddo
            enddo
            call MPI_SEND(ans, bcols, MPI_DOUBLE_PRECISION, master, row, MPI_COMM_WORLD, ierr)
         enddo
      endif
